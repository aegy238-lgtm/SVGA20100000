...
export const Store: React.FC<StoreProps> = ({ currentUser, onLoginRequired }) => {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      if (productsList.length > 0) {
        ...
        // If no products in DB, we could show MOCK or just empty.
        // For now, let's show MOCK if DB is empty so the UI isn't broken for new users,
        // but ideally the Admin should populate the DB.
        // setProducts(MOCK_PRODUCTS); 
        // Actually, let's show empty to respect "Control the store". 
        // If I want to control it, I must see the reality.
        setProducts([]);
      }
    } catch (err) {
  ...
  const categories = useMemo(() => {
    ...
    const allCats = new Set([...DEFAULT_CATEGORIES, ...dbCategories, ...Array.from(productCats)]);
    return ['الكل', ...Array.from(allCats)];
      ...
      {activeTab === 'products' ? (
        products.length === 0 && !loading ? (
          <div className="py-20 text-center bg-slate-950/40 rounded-[3rem] border border-white/5">
             <p className="text-slate-500 font-black uppercase tracking-widest text-xs">لا توجد منتجات متاحة حالياً</p>
          </div>
        ) : (
